...
!  $Author: samig $
!
! The bp_callback_ex code is free to copy, modify and distribute.
!
!-------------------------------------------------------------------------------!
! PURPOSE:                                                                      !
! Demonstration of the use of call-back procedure by the (mixed) integer        !
! progrmming solver.                                                            !
!                                                                               !
! Example of calling the (mixed) integer solver of glpk, using a call-back      !
! function (see cb_proc.f90), from Fortran in order to solve a binary problem.  !
! The example is a 10-variable problem (pp.58--65) from the book                !
! "Discrete Optimization" by D. R. Plane and C. McMillan, Jr., published by     !
! Prentice-Hall in 1971.                                                        !
!-------------------------------------------------------------------------------!
! $Revision: 1.2 $                 
! Last edited: $Date: 2013/10/01 22:28:57 $
...
PROGRAM bp_callback_ex  
  USE, INTRINSIC :: ISO_C_BINDING, ONLY: C_INT, C_DOUBLE, C_FUNLOC, C_NULL_CHAR

  USE glpk_fp

  USE cb_prob_common ! Module defining problem parameters
  ...
  INTEGER(C_INT) :: ia(0:NZ), ja(0:NZ)   ! ubd is NZ = number of nonzeros
  ...
  INTERFACE
     ...
     SUBROUTINE cb_proc(tree,info)  BIND(C)
       ...
       USE glpk_fp
       USE cb_prob_common, ONLY: N, prob_ptr, x 
       INTEGER(C_INT), VALUE :: tree, info 
  ...
  CALL glp_set_col_name(prob_ptr, 7, "X7"//C_NULL_CHAR)
  ...
  !Pass the address of the call-back function is to be used by the MIP solver

  !----------------------------------------------------------------------------------!

  ! The following compiles but then cb_proc (in fact glp_ios_reason) leads to segmentation 
  ! fault. Note that TYPE(C_FUNPTR) :: cb_func is used in the type declaration of glp_iocp. 

  prob_IPparam%cb_func = C_FUNLOC(cb_proc)   
